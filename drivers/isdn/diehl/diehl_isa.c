...
diehl_isa_irq(int irq, void *dev_id, struct pt_regs *regs) {
	...
	if (!card) {
		printk(KERN_WARNING "diehl_isa_irq: spurious interrupt %d\n", irq);
		return;
	}
	com = &card->shmem->com;
	...
	if ((tmp = readb(&com->Ind)) != 0) {
		...
		printk(KERN_DEBUG "diehl_ind Ind=%d\n", tmp);
...
diehl_isa_transmit(diehl_isa_card *card) {
	...
	if (!card) {
		printk(KERN_WARNING "diehl_isa_transmit: NULL card!\n");
		return;
	}
	com = &card->shmem->com;
	while ((skb = skb_dequeue(&((diehl_card *)card->card)->sndq))) {
		...
		if (skb->len > 269) {
			printk(KERN_WARNING "diehl_isa_transmit: skb > 269 bytes!!!\n");
			writeb(0, &com->XLock);
			return;
		}
...
diehl_isa_load(diehl_isa_card *card, diehl_isa_codebuf *cb) {
	...
	if (cbuf.bootstrap_len > 1024) {
		printk(KERN_WARNING "diehl_isa_boot: Invalid bootstrap size %ld\n",
		       cbuf.bootstrap_len);
		return -EINVAL;
	}
	if ((cbuf.boot_opt != DIEHL_ISA_BOOT_NORMAL) &&
	    (cbuf.boot_opt != DIEHL_ISA_BOOT_MEMCHK)) {
		printk(KERN_WARNING "diehl_isa_boot: Invalid bootstrap option %d\n",
		       cbuf.boot_opt);
		return -EINVAL;
	}
	if (!(code = kmalloc(cbuf.bootstrap_len + cbuf.firmware_len, GFP_KERNEL))) {
		printk(KERN_WARNING "diehl_isa_boot: Couldn't allocate code buffer\n");
		return -ENOMEM;
	}
	...
	switch (card->type & 0x0f) {
		...
		case DIEHL_CTYPE_PRI:
			...
			break;
		default:
			printk(KERN_WARNING "diehl_isa_boot: Unknown card type %d\n", card->type);
			kfree(code);
			return -EINVAL;
	}
		...
		if (((unsigned long)card->shmem < 0x0c0000) ||
			...
			printk(KERN_WARNING "diehl_isa_boot: illegal shmem: 0x%08lx\n",
			       (unsigned long)card->shmem);
			kfree(code);
			return -EINVAL;
		}
		/* Register shmem */
                if (check_shmem((unsigned long)card->shmem, card->ramsize)) {
                        printk(KERN_WARNING "diehl_isa_boot: memory at 0x%08lx already in use.\n",
                               (unsigned long)card->shmem);
			kfree(code);
                        return -EBUSY;
                }
	...
	if ((!card->ivalid) && card->master) {
		...
		if ((card->irq < 0) || (card->irq > 15) || 
			...
			printk(KERN_WARNING "diehl_isa_boot: illegal irq: %d\n", card->irq);
			diehl_isa_release_shmem(card);
			kfree(code);
			return -EINVAL;
		}
		/* Register irq */
		if (!request_irq(card->irq, &diehl_isa_irq, 0, "Diehl ISA ISDN", card))
			card->ivalid = 1;
		else {
			printk(KERN_WARNING "diehl_isa_boot: irq %d already in use.\n",
			       card->irq);
			diehl_isa_release_shmem(card);
			kfree(code);
			return -EBUSY;
	...
	if (cprimary != primary) {
		printk(KERN_WARNING "diehl_isa_boot: PRI check failed %d %d\n", primary, cprimary);
	...
	if (!check_signature((unsigned long)&card->shmem->c, code, 1020)) {
		printk(KERN_WARNING "diehl_isa_boot: Could not load bootcode\n");
		diehl_isa_release(card);
		kfree(code);
		return -EIO;
	}
	...
	if ((readb(&boot->ctrl) == 1) || (readb(&boot->ctrl) == 2)) {
		printk(KERN_WARNING "diehl_isa_boot: CPU start failed\n");
		diehl_isa_release(card);
		kfree(code);
		return -EIO;
	}
	...
	if (readb(&boot->ctrl) != 0) {
		printk(KERN_WARNING "diehl_isa_boot: CPU test failed\n");
		diehl_isa_release(card);
		kfree(code);
		return -EIO;
	}
	...
	if (readw(&boot->ebit)) {
		printk(KERN_WARNING "diehl_isa_boot: memory test failed (bit 0x%04x at 0x%08x)\n",
	...
	if (tmp != 8 && tmp != 16 && tmp != 24 &&
		...
		printk(KERN_WARNING "diehl_isa_boot: invalid memsize\n");
		diehl_isa_release(card);
		kfree(code);
		return -EIO;
	}
	...
	/* Download firmware */
	printk(KERN_INFO "diehl_isa_boot: Diehl %s %dkB, loading firmware ...\n", 
	       diehl_ctype_name[card->type],
	       tmp * 16);
	...
	while (tmp--) {
		...
		if (readb(&boot->ctrl)) {
			printk(KERN_WARNING "diehl_isa_boot: dowload timeout\n");
			diehl_isa_release(card);
			kfree(code);
			return -EIO;
		}
	...
	if (readw(&boot->signature) != 0x4447) {
		printk(KERN_WARNING "diehl_isa_boot: firmware selftest failed %04x\n",
	...
	if (card->irqprobe == 1) {
		printk(KERN_WARNING "diehl_isa_boot: IRQ test failed\n");
		diehl_isa_release(card);
		return -EIO;
	}
