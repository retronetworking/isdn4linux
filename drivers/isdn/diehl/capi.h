/* $Id$
 *
 * ISDN lowlevel-module for the Diehl active cards.
 *
...
/* Header for Indications queued in rcvq */
typedef struct diehl_indhdr {
	__u8 ret;
	__u8 id;
	__u8 ch;
	__u8 more;
...
/* Ack-Data from card queued in rackq */
typedef struct diehl_ack {
	__u8 ret;
	__u8 id;
	__u8 ch;
} diehl_ack;
...
typedef struct capi_msgcmd {
	__u8 cmd;
	__u8 subcmd;
} capi_msgcmd;

/* CAPI message header */
typedef struct capi_msghdr {
	__u16 len;
	__u16 applicationID;
	capi_msgcmd cmd;
	__u16 msgnum;
...
typedef struct capi_msgdsc {
	capi_msgcmd cmd;
	char *description;
} capi_msgdsc;

/* CAPI Adress */
typedef struct capi_addr {
...
typedef  union capi_infonr {              /* info number                  */
...
/* CAPI INFO element */
typedef union  capi_infoel {              /* info element                 */
...
typedef struct capi_msg {
	...
	union msg {
		...
		struct connect_b3_resp {
			__u16 ncci;
			__u8  rejectcause;
			capi_ncpi ncpi __attribute__ ((packed));
		} connect_b3_resp;
		...
		struct select_b2_protocol_req {
			__u16 plci;
			__u8  protocol;
			capi_dlpd dlpd __attribute__ ((packed));
		...
		struct select_b3_protocol_req {
			__u16 plci;
			__u8  protocol;
			capi_ncpd ncpd __attribute__ ((packed));
...
capi_nextsmsg(act2000_card *card)
{
	unsigned long flags;
	unsigned short n;
...
extern int capi_chkhdr(act2000_card *, capi_msghdr *);
extern int capi_listen_req(act2000_card *);
extern int capi_manufacturer_req_net(act2000_card *);
extern int capi_manufacturer_req_v42(act2000_card *, ulong);
extern int capi_manufacturer_req_errh(act2000_card *);
extern int capi_manufacturer_req_msn(act2000_card *);
extern int capi_connect_req(act2000_card *, act2000_chan *, char *, char, int, int);
extern void capi_select_b2_protocol_req(act2000_card *, act2000_chan *);
extern void capi_disconnect_b3_req(act2000_card *, act2000_chan *);
extern void capi_connect_resp(act2000_card *, act2000_chan *, __u8);
extern void capi_dispatch(act2000_card *);
#ifdef DEBUG_MSG
extern void capi_debug_msg(struct sk_buff *skb, int);
#else
#define capi_debug_msg(skb, len)
#endif
#endif
