			 ...
			 { if (cr == -1) 
                        ...
                        if ((pc->prot.dss1.invoke_id) && (pc->prot.dss1.invoke_id == id))
                          { /* Diversion successfull */
                            ...
                            l3pc_l3l4(pc, CC_REDIR | INDICATION, 0);                                  } /* Diversion successfull */
                        else
			 ...
			 { if (cr == -1)
                        ...
                        if ((pc->prot.dss1.invoke_id) && (pc->prot.dss1.invoke_id == id))
                          { /* Deflection error */
                            ...
                            pc->prot.dss1.invoke_id = 0; 
                            pc->redir_result = pc->prot.dss1.remote_result; 
                            l3pc_l3l4(pc, CC_REDIR | INDICATION, 0);  
                          } /* Deflection error */
                        else
		   ...
		   void *arg)
{
	...
	l3pc_l3l4(pc, CC_PROCEED_SEND | INDICATION, 0); 
}
...
l3dss1_progress(struct l3_process *pc, u_char pr, void *arg)
{
	...
	if (err)
		l3dss1_std_ie_err(pc, err);
	if (ERR_IE_COMPREHENSION != err)
		l3pc_l3l4(pc, CC_PROGRESS | INDICATION, 0);
}
...
l3dss1_notify(struct l3_process *pc, u_char pr, void *arg)
{
	...
	if (err)
		l3dss1_std_ie_err(pc, err);
	if (ERR_IE_COMPREHENSION != err)
		l3pc_l3l4(pc, CC_NOTIFY | INDICATION, 0);
}
...
l3dss1_release_ind(struct l3_process *pc, u_char pr, void *arg)
{
	...
	if (callState == 0) {
		...
		l3pc_l3l4(pc, CC_IGNORE | INDICATION, 0);
	}
}
...
l3dss1_suspend_ack(struct l3_process *pc, u_char pr, void *arg)
{
	...
	newl3state(pc, 0);
	pc->para.cause = NO_CAUSE;
	l3pc_l3l4(pc, CC_SUSPEND | CONFIRM, 0);
...
l3dss1_resume_ack(struct l3_process *pc, u_char pr, void *arg)
{
	...
	L3DelTimer(&pc->timer);
	l3pc_l3l4(pc, CC_RESUME | CONFIRM, 0);
	newl3state(pc, 10);
	if (ret) /* STATUS for none mandatory IE errors after actions are taken */
		l3dss1_std_ie_err(pc, ret);
}
