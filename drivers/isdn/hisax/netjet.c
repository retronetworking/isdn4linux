...
mode_tiger(struct BCState *bcs, int mode, int bc)
{
	...
	debugl1(L1_DEB_HSCX, cs, "Tiger mode %d bchan %d/%d",
		mode, bc, bcs->channel);
	bcs->mode = mode;
	bcs->channel = bc;
	switch (mode) {
		case (B1_MODE_NULL):
			...
			debugl1(L1_DEB_HSCX, cs, "Tiger stat rec %d/%d send %d",
	...
	debugl1(L1_DEB_HSCX, cs, "tiger: set %x %x %x  %x/%x  pulse=%d",
...
static int make_raw_data(struct BCState *bcs) {
	...
	if (!bcs->tx_skb) {
		debugl1(L1_DEB_WARN, bcs->cs, "tiger make_raw: NULL skb");
		return(1);
	}
	...
	debugl1(L1_DEB_HSCX, bcs->cs,"tiger make_raw: in %ld out %d.%d",
		...
		for (j=0;j<8;j++) {
			if (state == HDLC_ZERO_SEARCH) {
				if (val & 1) {
					...
					debugl1(L1_DEB_HSCX, bcs->cs,"tiger read_raw: zBit(%d,%d,%d) %x",
				...
				if (val & 1) {
					...
					if (r_one==6) {
						...
						debugl1(L1_DEB_HSCX, bcs->cs,"tiger read_raw: flag(%d,%d,%d) %x",
							bcs->hw.tiger.r_tot,i,j,val);
					}
					r_one=0;
				}
				...
				if ((state != HDLC_ZERO_SEARCH) &&
					...
					debugl1(L1_DEB_HSCX, bcs->cs,"tiger read_raw: byte1(%d,%d,%d) rval %x val %x i %x",
				...
				if (val & 1) {
					...
					if (r_one==6) {
						...
						bitcnt++;
						if (bitcnt & 7) {
							debugl1(L1_DEB_WARN, bcs->cs, "tiger: frame not byte aligned");
							state=HDLC_FLAG_SEARCH;
							bcs->hw.tiger.r_err++;
							...
							debugl1(L1_DEB_HSCX, bcs->cs,"tiger frame end(%d,%d): fcs(%x) i %x",
								i,j,bcs->hw.tiger.r_fcs, bcs->cs->hw.njet.irqstat0);
							if (bcs->hw.tiger.r_fcs == PPP_GOODFCS) {
								got_frame(bcs, (bitcnt>>3)-3);
							} else {
								if (bcs->cs->debug) {
									debugl1(L1_DEB_WARN, bcs->cs, "tiger FCS error");
				...
				if ((state == HDLC_FRAME_FOUND) &&
					...
					if ((bitcnt>>3)>=HSCX_BUFMAX) {
						debugl1(L1_DEB_WARN, bcs->cs, "tiger: frame too big");
						r_val=0; 
						state=HDLC_FLAG_SEARCH;
						bcs->hw.tiger.r_err++;
#ifdef ERROR_STATISTIC
...
static void read_tiger(struct IsdnCardState *cs) {
	...
	if ((cs->hw.njet.irqstat0 & cs->hw.njet.last_is0) & NETJET_IRQM0_READ) {
		debugl1(L1_DEB_WARN, cs,"tiger warn read double dma %x/%x",
...
static void fill_dma(struct BCState *bcs)
{
	...
	if (!bcs->tx_skb)
		return;
	debugl1(L1_DEB_HSCX, bcs->cs,"tiger fill_dma1: c%d %4x", bcs->channel,
		bcs->Flag);
	if (test_and_set_bit(BC_FLG_BUSY, &bcs->Flag))
		return;
	if (make_raw_data(bcs))
		return;		
	debugl1(L1_DEB_HSCX, bcs->cs,"tiger fill_dma2: c%d %4x", bcs->channel,
		bcs->Flag);
	...
	debugl1(L1_DEB_HSCX, bcs->cs,"tiger fill_dma3: c%d %4x", bcs->channel,
		bcs->Flag);
}
...
static void write_raw(struct BCState *bcs, u_int *buf, int cnt) {
	...
	if (test_bit(BC_FLG_BUSY, &bcs->Flag)) {
		...
		debugl1(L1_DEB_HSCX, bcs->cs,"tiger write_raw: c%d %x-%x %d/%d %d %x", bcs->channel,
		...
		if (!bcs->hw.tiger.sendcnt) {
			if (!bcs->tx_skb) {
				debugl1(L1_DEB_WARN, bcs->cs,"tiger write_raw: NULL skb s_cnt %d", s_cnt);
			} else {
			...
			if ((bcs->tx_skb = skb_dequeue(&bcs->squeue))) {
				...
				if (s_cnt < cnt) {
					...
					debugl1(L1_DEB_HSCX, bcs->cs, "tiger write_raw: fill rest %d",
						cnt - s_cnt);
				}
		...
		debugl1(L1_DEB_HSCX, bcs->cs,"tiger write_raw: fill half");
	} else if (test_and_clear_bit(BC_FLG_HALF, &bcs->Flag)) {
		test_and_set_bit(BC_FLG_EMPTY, &bcs->Flag);
		fill_mem(bcs, buf, cnt, bcs->channel, 0xff);
		debugl1(L1_DEB_HSCX, bcs->cs,"tiger write_raw: fill full");
	}
}
...
static void write_tiger(struct IsdnCardState *cs) {
	...
	if ((cs->hw.njet.irqstat0 & cs->hw.njet.last_is0) & NETJET_IRQM0_WRITE) {
		debugl1(L1_DEB_WARN, cs,"tiger warn write double dma %x/%x",
			cs->hw.njet.irqstat0, cs->hw.njet.last_is0);
#ifdef ERROR_STATISTIC
		if (cs->bcs[0].mode)
			cs->bcs[0].err_tx++;
...
inittiger(struct IsdnCardState *cs))
{
	...
	debugl1(L1_DEB_WARN, cs, "tiger: send buf %x - %x", (u_int)cs->bcs[0].hw.tiger.send,
	...
	debugl1(L1_DEB_WARN, cs, "tiger: rec buf %x - %x", (u_int)cs->bcs[0].hw.tiger.rec,
	...
	debugl1(L1_DEB_WARN, cs, "tiger: dmacfg  %x/%x  pulse=%d",
		inl(cs->hw.njet.base + NETJET_DMA_WRITE_ADR),
		inl(cs->hw.njet.base + NETJET_DMA_READ_ADR),
		bytein(cs->hw.njet.base + NETJET_PULSE_CNT));
...
netjet_interrupt(int intno, void *dev_id, struct pt_regs *regs)
{
	...
	if (!((sval = bytein(cs->hw.njet.base + NETJET_IRQSTAT1)) &
		...
		debugl1(L1_DEB_ISAC, cs, "tiger: i1 %x %x", sval, val);
		if (val) {
