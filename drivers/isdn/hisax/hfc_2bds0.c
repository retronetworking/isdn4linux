...
SelFiFo(struct IsdnCardState *cs, u_char FiFo)
{
	...
	switch(FiFo) {
		...
		default:
			restore_flags(flags);
			debugl1(L1_DEB_WARN, cs, "SelFiFo Error");
			return(0);
	}
...
*hfc_empty_fifo(struct BCState *bcs, int count)
{
	...
	if ((cs->debug & L1_DEB_HSCX) && !(cs->debug & L1_DEB_HSCX_FIFO))
		debugl1(L1_DEB_HSCX, cs, "hfc_empty_fifo");
	idx = 0;
	save_flags(flags);
	if (count > HSCX_BUFMAX + 3) {
		debugl1(L1_DEB_WARN, cs, "hfc_empty_fifo: incoming packet too large");
		...
		debugl1(L1_DEB_WARN, cs, "hfc_empty_fifo: incoming packet too small");
		cip = HFCB_FIFO | HFCB_FIFO_OUT | HFCB_REC | HFCB_CHANNEL(bcs->channel);
		...
		if (idx != count - 3) {
			sti();
			debugl1(L1_DEB_WARN, cs, "RFIFO BUSY error");
			...
			sti();
			debugl1(L1_DEB_HSCX, cs, "hfc_empty_fifo %d chksum %x stat %x",
				bcs->channel, chksum, stat);
			if (stat) {
				debugl1(L1_DEB_WARN, cs, "FIFO CRC error");
				idev_kfree_skb_irq(skb, FREE_READ);
				skb = NULL;
#ifdef ERROR_STATISTIC
				bcs->err_crc++;
...
hfc_fill_fifo(struct BCState *bcs)
{
	...
	sti();
	debugl1(L1_DEB_HSCX, cs, "hfc_fill_fifo %d f1(%d) f2(%d) z1(%x)",
	...
	if (fcnt < 0)
		fcnt += 32;
	if (fcnt > 30) {
		debugl1(L1_DEB_HSCX, cs, "hfc_fill_fifo more as 30 frames");
		restore_flags(flags);
		return;
	}
	count = GetFreeFifoBytes_B(bcs);
	debugl1(L1_DEB_HSCX, cs, "hfc_fill_fifo %d count(%ld/%d),%lx",
		bcs->channel, bcs->tx_skb->len,	count, current->state);
	if (count < bcs->tx_skb->len) {
		debugl1(L1_DEB_HSCX, cs, "hfc_fill_fifo no fifo mem");
		restore_flags(flags);
		return;
	}
	...
	if (idx != bcs->tx_skb->len) {
		sti();
		debugl1(L1_DEB_WARN, cs, "FIFO Send BUSY error");
...
main_rec_2bds0(struct BCState *bcs)
{
	...
	sti();
	if (f1 != f2) {
		debugl1(L1_DEB_HSCX, cs, "hfc rec %d f1(%d) f2(%d)",
			bcs->channel, f1, f2);
		cli();
		...
		rcnt++;
		debugl1(L1_DEB_HSCX, cs, "hfc rec %d z1(%x) z2(%x) cnt(%d)",
...
mode_2bs0(struct BCState *bcs, int mode, int bc)
{
	...
	debugl1(L1_DEB_HSCX, cs, "HFCD bchannel mode %d bchan %d/%d",
		mode, bc, bcs->channel);
	bcs->mode = mode;
	bcs->channel = bc;
	switch (mode) {
...
int receive_dmsg(struct IsdnCardState *cs)
{
	...
	while ((f1 != f2) && count--) {
		...
		rcnt++;
		debugl1(L1_DEB_ISAC, cs, "hfcd recd f1(%d) f2(%d) z1(%x) z2(%x) cnt(%d)",
			f1, f2, z1, z2, rcnt);
		sti();
		idx = 0;
		cip = HFCD_FIFO | HFCD_FIFO_OUT | HFCD_REC;
		if (rcnt > MAX_DFRAME_LEN + 3) {
			debugl1(L1_DEB_WARN, cs, "empty_fifo d: incoming packet too large");
			while (idx < rcnt) {
				cli();
				if (!(WaitNoBusy(cs)))
					break;
			...
			debugl1(L1_DEB_WARN, cs, "empty_fifo d: incoming packet too small");
			cli();
			...
			if (idx != (rcnt - 3)) {
				sti();
				debugl1(L1_DEB_WARN, cs, "RFIFO D BUSY error");
				...
				stat = ReadReg(cs, HFCD_DATA, cip);
				sti();
				debugl1(L1_DEB_ISAC, cs, "empty_dfifo chksum %x stat %x",
					chksum, stat);
				if (stat) {
					debugl1(L1_DEB_WARN, cs, "FIFO CRC error");
					idev_kfree_skb_irq(skb, FREE_READ);
...
hfc_fill_dfifo(struct IsdnCardState *cs)
{
	...
	sti();
	debugl1(L1_DEB_ISAC, cs, "hfc_fill_Dfifo f1(%d) f2(%d) z1(%x)",
	...
	if (fcnt < 0)
		fcnt += 16;
	if (fcnt > 14) {
		debugl1(L1_DEB_HSCX, cs, "hfc_fill_Dfifo more as 14 frames");
		restore_flags(flags);
		return;
	}
	count = GetFreeFifoBytes_D(cs);
	debugl1(L1_DEB_ISAC, cs, "hfc_fill_Dfifo count(%ld/%d)",
		cs->tx_skb->len, count);
	if (count < cs->tx_skb->len) {
		debugl1(L1_DEB_ISAC, cs, "hfc_fill_Dfifo no fifo mem");
		restore_flags(flags);
		return;
	}
	...
	if (idx != cs->tx_skb->len) {
		sti();
		debugl1(L1_DEB_WARN, cs, "DFIFO Send BUSY error");
...
hfc2bds0_interrupt(struct IsdnCardState *cs, u_char val)
{
	...
	debugl1(L1_DEB_ISAC, cs, "HFCD irq %x %s", val,
		test_bit(FLG_LOCK_ATOMIC, &cs->HW_Flags) ?
		"locked" : "unlocked");
	val &= cs->hw.hfcD.int_m1;
	if (val & 0x40) { /* TE state machine irq */
		exval = cs->readisac(cs, HFCD_STATES) & 0xf;
		debugl1(L1_DEB_ISAC, cs, "ph_state chg %d->%d", cs->dc.hfcd.ph_state,
			exval);
	...
	while (val) {
		...
		if (val & 0x08) {
			if (!(bcs=Sel_BCS(cs, 0))) {
				debugl1(L1_DEB_WARN, cs, "hfcd spurious 0x08 IRQ");
			} else 
				main_rec_2bds0(bcs);
		}
		if (val & 0x10) {
			if (!(bcs=Sel_BCS(cs, 1))) {
				debugl1(L1_DEB_WARN, cs, "hfcd spurious 0x10 IRQ");
			} else 
				main_rec_2bds0(bcs);
		}
		if (val & 0x01) {
			if (!(bcs=Sel_BCS(cs, 0))) {
				debugl1(L1_DEB_WARN, cs, "hfcd spurious 0x01 IRQ");
			} else {
				if (bcs->tx_skb) {
					if (!test_and_set_bit(FLG_LOCK_ATOMIC, &cs->HW_Flags)) {
						hfc_fill_fifo(bcs);
		...
		if (val & 0x02) {
			if (!(bcs=Sel_BCS(cs, 1))) {
				debugl1(L1_DEB_WARN, cs, "hfcd spurious 0x02 IRQ");
			} else {
				if (bcs->tx_skb) {
					if (!test_and_set_bit(FLG_LOCK_ATOMIC, &cs->HW_Flags)) {
						hfc_fill_fifo(bcs);
			...
			if (cs->tx_skb) {
				if (cs->tx_skb->len) {
					if (!test_and_set_bit(FLG_LOCK_ATOMIC, &cs->HW_Flags)) {
						...
						debugl1(L1_DEB_WARN, cs, "hfc_fill_dfifo irq blocked");
					}
					goto afterXPR;
				} else {
      ...
      afterXPR:
		if (cs->hw.hfcD.int_s1 && count--) {
			val = cs->hw.hfcD.int_s1;
			cs->hw.hfcD.int_s1 = 0;
			debugl1(L1_DEB_ISAC, cs, "HFCD irq %x loop %d", val, 15-count);
		} else
			val = 0;
		restore_flags(flags);
	}
		...
		case (PH_DATA | REQUEST):
			...
			if (cs->tx_skb) {
				...
				if (!test_and_set_bit(FLG_LOCK_ATOMIC, &cs->HW_Flags)) {
					...
					debugl1(L1_DEB_WARN, cs, "hfc_fill_dfifo blocked");
...
#endif
		default:
			debugl1(L1_DEB_WARN, cs, "hfcd_l1hw unknown pr %4x", pr);
			break;
	}
}
