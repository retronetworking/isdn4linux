...
hfc_clear_fifo(struct BCState *bcs)
{
	...
	if ((cs->debug & L1_DEB_HSCX) && !(cs->debug & L1_DEB_HSCX_FIFO))
		debugl1(L1_DEB_HSCX, cs, "hfc_clear_fifo");
	save_flags(flags);
	cli();
	...
	while (((f1 != f2) || (z1 != z2)) && cnt--) {
		debugl1(L1_DEB_HSCX, cs, "hfc clear %d f1(%d) f2(%d)",
			bcs->channel, f1, f2);
		rcnt = z1 - z2;
		if (rcnt < 0)
		...
		if (rcnt)
			rcnt++;
		debugl1(L1_DEB_HSCX, cs, "hfc clear %d z1(%x) z2(%x) cnt(%d)",
...
hfc_empty_fifo(struct BCState *bcs, int count)
{
	...
	if ((count < 4) && (bcs->mode != B1_MODE_TRANS)) {
...
hfc_fill_fifo(struct BCState *bcs)
{
	...
	debugl1(L1_DEB_HSCX, cs, "hfc_fill_fifo %d count(%ld/%d)",
		bcs->channel, bcs->tx_skb->len,	count);
...
main_irq_hfc(struct BCState *bcs)
{
	...
	if (receive || (bcs->mode == B1_MODE_TRANS)) {
		WaitForBusy(cs);
		z1 = ReadZReg(bcs, HFC_Z1 | HFC_REC | HFC_CHANNEL(bcs->channel));
		z2 = ReadZReg(bcs, HFC_Z2 | HFC_REC | HFC_CHANNEL(bcs->channel));
		rcnt = z1 - z2;
...
mode_hfc(struct BCState *bcs, int mode, int bc)
{
	...
	debugl1(L1_DEB_HSCX, cs, "HFC 2BS0 mode %d bchan %d/%d",
		mode, bc, bcs->channel);
	bcs->mode = mode;
	bcs->channel = bc;
...
hfc_l2l1(struct PStack *st, int pr, void *arg)
{
	...
	switch (pr) {
		...
		case (PH_DEACTIVATE | CONFIRM):
			...
			mode_hfc(st->l1.bcs, B1_MODE_NULL, st->l1.bc);
...
{
	mode_hfc(bcs, B1_MODE_NULL, bcs->channel);
...
inithfc(struct IsdnCardState *cs))
{
	...
	mode_hfc(cs->bcs, B1_MODE_NULL, 0);
	mode_hfc(cs->bcs + 1, B1_MODE_NULL, 0);
}
