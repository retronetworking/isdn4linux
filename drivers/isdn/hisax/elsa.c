...
initelsa(struct IsdnCardState *sp)
{
	...
	sprintf(tmp, "IRQ %d count %d", sp->irq, irq_cnt);
	debugl1(sp, tmp);
	...
	if (ret) {
		...
		timeout += 111;
		while (timeout != jiffies);  /* Warte 1,11 s */
		restore_flags(flags);
		printk(KERN_INFO "Elsa: %d timer tics in 1.11 sek\n",
			sp->counter);
		if (abs(sp->counter-125)<2) {
...
probe_elsa_adr(unsigned int adr)
{
	...
	long flags;
		...
		printk(KERN_WARNING
			"Probing ELSA Port 0x%x: already in use\n",
                               	adr);
                return(0);
        }
	save_flags(flags);
...
setup_elsa(struct IsdnCard *card)
{
	...
	save_flags(flags);
	sti();
	while (timout != jiffies);  /* Warte auf Startflanke */
	bytein(sp->cfg_reg +CARD_START_TIMER);
