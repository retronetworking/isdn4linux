 ...
 *
 * 2001-02-24 make it work with new kernelcapi (b1pci only)
...
int b1_load_and_config(struct capi_ctr *ctr, void *data)
{
	avmb1_loadandconfigdef ldef;
	capiloaddata ldata;
	int retval;
	
	if (copy_from_user(&ldef, data, sizeof(avmb1_loadandconfigdef)))
		return -EFAULT;
	...
	if (ctr->cardstate != CARD_DETECTED) {
		printk(KERN_INFO "kcapi: load: contr=%d not in detect state\n", ldef.contr);
		return -EBUSY;
	}
	ctr->cardstate = CARD_LOADING;
	
	retval = b1_load_firmware(ctr, &ldata);
	
	if (retval) {
		ctr->cardstate = CARD_DETECTED;
		return retval;
	}
	
	while (ctr->cardstate != CARD_RUNNING) {
...
int b1_reset_ctr(struct capi_ctr *ctrl)
{
	avmctrl_info *cinfo = (avmctrl_info *)(ctrl->driverdata);
	avmcard *card = cinfo->card;
	unsigned int port = card->port;
...
EXPORT_SYMBOL(b1_load_and_config);
