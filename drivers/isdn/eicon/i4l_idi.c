...
idi_assign_req(eicon_REQ *reqbuf, int signet, eicon_chan *chan)
{
  ...
  if (!signet) {
	...
	switch(chan->l2prot) {
#if 0
                case ISDN_PROTO_L2_V120:
                       reqbuf->XBuffer.P[l++] = 8; /* V.120 */
                       break;
#endif
	...
	switch(chan->l3prot) {
       ...
       switch(chan->l2prot) {
               ...
               case ISDN_PROTO_L2_V120:
                    ...
                    reqbuf->XBuffer.P[l++] = 0x08;         /* address A */
                    reqbuf->XBuffer.P[l++] = 0x01;         /* address B */
                    reqbuf->XBuffer.P[l++] = 127;          /* mode */
                    reqbuf->XBuffer.P[l++] = 7;            /* window size */
                break;
#endif
               case ISDN_PROTO_L2_X75I:
               case ISDN_PROTO_L2_X75UI:
...
#if 0
                                   if ((chan->l2prot == ISDN_PROTO_L2_V120) && (skb->len)) {
                                          while((skb->len > 1) && (!(skb->data[0] & 0x80)))
                                            skb_pull(skb, 1);
                                          eicon_log(ccard, 128, "idi_rcv: Ch%d: V120 %d bytes after patch\n", chan->No, skb->len);
                                    }
#endif
					ccard->interface.rcvcallb_skb(ccard->myid, chan->No, skb);
					free_buff = 0; 
...
#if 0
        if (chan->l2prot == ISDN_PROTO_L2_V120) {
            if (!skb_headroom(skb)) {
               eicon_log(card, 1, "idi_snd: Ch%d: no skb headroom for V120\n",
                     chan->No);
            } else {
                skb_push(skb, 1);
                skb->data[0] = 0x83;
             eicon_log(card, 128, "idi_snd: Ch%d: V120 %d bytes after patch\n", chan->No, skb->len);
            }
        }
#endif
	while(offset < len) {
