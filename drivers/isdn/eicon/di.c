...
/* file: di.c                                                       */
/*                                                                  */
/* direct interface library for basic rate and primary rate         */
/* adapters                                                         */
/* Copyright (c) G. Diehl ISDN GmbH 1992-1998                       */
/*------------------------------------------------------------------*/
#include "platform.h"
#include "pc.h"
#include "pr_pc.h"
  ...
  if((Rc &0xf0)==ASSIGN_RC) {
    ...
    if(e_no) {
      ...
      if ((Rc == ASSIGN_OK) && a->ram_offset &&
          ...
          ((extended_info_type == DIVA_RC_TYPE_STREAM_PTR) ||
        ...
        if (extended_info_type == DIVA_RC_TYPE_STREAM_PTR) {
          a->stream_irq[Id] = extended_info+8;
          DBG_TRC(("Id=0x%x STREAMS=%08x:%08x",
...
#if defined(DIVA_ISTREAM)
      if (a->rx_stream[this->Id] && (!a->stream_irq[this->Id]) &&
          (Ind == N_DATA)) {
        int final = 0;
