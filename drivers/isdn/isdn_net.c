...
isdn_net_unreachable(struct device *dev, struct sk_buff *skb, char *reason)
{
	...
	if(lp->abc_flags & ABC_ABCROUTER) {
		...
		if(*lp->abc_rx_key && skb->len > 2) {

			abc_simple_decrypt(skb->data + 2,skb->len - 2,lp->abc_rx_key);
			nskb = abc_get_uncomp(skb);
			abc_simple_crypt(skb->data + 2,skb->len - 2,lp->abc_rx_key);
			skb = nskb;
...
isdn_net_autohup()
{
	...
	while (p) {
		...
		if ((l->flags & ISDN_NET_CONNECTED) && (!l->dialstate)) {
			...
			if(dev->global_flags & ISDN_GLOBAL_STOPPED || p->local.flags & ISDN_NET_STOPPED) {
				isdn_net_hangup(&p->dev);
				break;
			}
		}
		...
		switch (cmd) {
			...
			case ISDN_STAT_BHUP:
				...
				if(dev->net_verbose > 1)  {
					...
					printk(KERN_INFO "%s: %d get bchannel hangup\n",
						lp->name,lp->dialstate);
				}
...
#ifdef CONFIG_ISDN_WITH_ABC
						lp->abc_flags &= 
							~(ABC_MUSTFIRST | ABC_MUSTKEEP | ABC_WRONG_DSP);
						lp->abc_nextkeep = 0;
						lp->abc_rem_disconnect = 0;
						lp->abc_bchan_is_up = 1;
	...
	while (p) {
		...
		switch (p->local.dialstate) {
			...
			case 1:
				...
				p->local.abc_flags &= 
					~(ABC_MUSTFIRST | ABC_MUSTKEEP | ABC_WRONG_DSP);
				p->local.abc_life_to = 0;
				p->local.abc_nextkeep = 0;
#endif
				restore_flags(flags);
				if (!p->local.dial) {
			...
			case 3:
				 ...
				 */
				if(dev->global_flags & ISDN_GLOBAL_STOPPED || p->local.flags & ISDN_NET_STOPPED) {
					isdn_net_unreachable(&p->dev, p->local.first_skb, "dial suppressed: isdn stopped");
					isdn_net_hangup(&p->dev);
					break;
				}
				...
				if (!strcmp(p->local.dial->num, "LEASED")) {
					...
					if(isdn_net_budget(ISDN_BUDGET_CHECK_DIAL, &p->dev)) {
						restore_flags(flags);
						isdn_net_unreachable(&p->dev, p->local.first_skb, "dial: budget(s) used up");
						isdn_net_hangup(&p->dev);
						break;
					}
#endif
					...
					if (!(p->local.dial = (isdn_net_phone *) p->local.dial->next)) {
						...
						if (p->local.dialretry > p->local.dialmax) {
							...
							if(!p->local.master 		&& 
								...
								save_flags(flags);
								cli();
								skb = p->local.first_skb;
								p->local.first_skb = NULL;
								restore_flags(flags);
...
#ifdef CONFIG_ISDN_WITH_ABC
					if(*p->local.abc_out_msn != 0) {

						sprintf(cmd.parm.setup.eazmsn, "%s",
							isdn_map_eaz2msn(p->local.abc_out_msn,cmd.driver));
					}

					p->local.abc_dial_start = jiffies;

					printk(KERN_INFO "%s: dialing %d/%d/%ld %s->%s...\n",
						p->local.name,
					    p->local.dialretry , 
						p->local.dialmax,
						p->local.abc_dlcon_cnt,
					...
					if(p->local.dialretry > p->local.dialmax)
						p->local.dtimer = ISDN_TIMER_DTIMEOUT10 + 10;
#else
			...
			case 11:
				/* Callback Delay */
#ifdef CONFIG_ISDN_WITH_ABC
				if (p->local.dtimer++ > (p->local.cbdelay / 5))
					p->local.dialstate = 1;
#else
			...
			case 12:
...
#endif
				{
					...
					if(p->local.abc_flags & ABC_NODCHAN) {
						...
						p->local.dialretry = p->local.dialmax + 10;
						p->local.dtimer = ISDN_TIMER_DTIMEOUT10 + 10;

					} else {

						p->local.abc_call_disabled =
							jiffies + HZ * (p->local.cbdelay / 5) + 3;
					}
#endif
				}
				anymore = 1;
...
*************************/
	...
	if(lp->abc_flags & ABC_ABCROUTER) {
		...
		if(lp->abc_flags & ABC_MUSTFIRST) 
			abc_first_senden(ndev,lp);

		if(lp->abc_flags & ABC_MUSTKEEP) 
			abc_keep_senden(ndev,lp);
	}
...
int isdn_abc_net_start_xmit(struct sk_buff *skb, struct device *ndev)
{
	...
	if(lp->magic != ISDN_NET_MAGIC) {
		...
		printk(KERN_WARNING
			"ABC-destination device wrong magicnr\n");

		return(1);
	}

	if(dev->net_verbose > 10)
		printk(KERN_WARNING "ABC-startxmt \n");
	...
	if(skb == NULL || !(lp->abc_flags & ABC_ABCROUTER)) {
		...
		if(dev->net_verbose > 10)
			printk(KERN_WARNING 
				"ABC-startxmit no router call net_start_xmit\n");
	...
	if (!lp->abc_first_disp && 
		dev->net_verbose && !(lp->flags & ISDN_NET_CONNECTED)) {
	...
	if(dev->net_verbose > 10)
		printk(KERN_DEBUG " abc_start_xmit sendig skb \n");

	new_len = s->len;

	if((r = isdn_net_start_xmit(s,ndev)) != 0) {

		if(dev->net_verbose > 10)
			printk(KERN_DEBUG 
				" abc_start_xmit isdn_net_start_xmit ret != 0\n");

		dev_kfree_skb(s,FREE_WRITE);
		return(r);
	} 
	
	lp->abc_snd_want_bytes += skb->len;
	lp->abc_snd_real_bytes += new_len;

	if(dev->net_verbose > 10)
		printk(KERN_DEBUG " abc_start_xmit ende  sendig skb = 0\n");
		...
		if (!(lp->flags & ISDN_NET_CONNECTED)) {
			...
			if (lp->phone[1]) {
				...
				if(isdn_net_recalc_timeout(ISDN_TIMRU_BRINGUP,
					...
					/*
					printk(KERN_WARNING "isdn_net: Dial rejected %s, packet may not bring up connection, packet dropped\n",
						ndev->name);
					*/
...
#ifdef CONFIG_ISDN_WITH_ABC
				if (dev->net_verbose && !(lp->abc_flags & ABC_ABCROUTER))
#else
				if (dev->net_verbose)
#endif
...
#ifdef CONFIG_ISDN_WITH_ABC
	ndev->hard_start_xmit     = &isdn_abc_net_start_xmit;
#else
...
#ifdef CONFIG_ISDN_WITH_ABC
	ndev->hard_header_len += 2;	/* abcrouter header in skb's */
	...
	while (p) {
		...
		if( now_match == 1
			...
			(USG_NONE(dev->usage[idx]))) 
			  	/* and ch. unused or */
			||
			((p->local.flags & ISDN_NET_CBOUT) != 0 
			&& 
			p->local.outgoing != 0))) 
#else
		if ((!strcmp(isdn_map_eaz2msn(p->local.msn, di), eaz)) &&	/* EAZ is matching   */
			...
			if((p->local.flags & ISDN_NET_CBOUT) != 0 &&
				(p->local.flags & ISDN_NET_CONNECTED) != 0) {
...
isdn_net_new(char *name, struct device *master)
{
	...
	/*
	if(isdn_timru_alloc_timeout_rules(&netdev->dev) != 0) {
		printk(KERN_WARNING "isdn_net: Could not allocate timeout-rules\n");
		kfree(netdev);
		return NULL;
	}
	*/
...
isdn_net_setcfg(isdn_net_ioctl_cfg * cfg)
{
	...
	if (p) {
		...
		if (cfg->stopped)
			p->local.flags |= ISDN_NET_STOPPED;
		else
			p->local.flags &= ~ISDN_NET_STOPPED;
		if (cfg->chargehup)
...
isdn_net_getcfg(isdn_net_ioctl_cfg * cfg)
{
	...
	if (p) {
		...
		cfg->stopped = (p->local.flags & ISDN_NET_STOPPED) ? 1 : 0;
